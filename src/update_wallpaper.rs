...
pub fn update_wallpaper(deadline: &str) {
    let file_path = generate_wallpaper(deadline);
...
fn generate_wallpaper(deadline: &str) -> String {
    ...
    let text_png = renderer
        .render_text_to_png_data(deadline, 100, "ffffff")
        .unwrap();
